...
export default function AlbumSharing() {
  ...
  // The code the owner hands out: the freshly created one, else the persisted
  // link code when revisiting an existing share. Empty for joiners.
  const shareCode = code || (link?.role === 'owner' ? (link?.code ?? '') : '');

  // Regenerate the QR whenever that code changes; clear it otherwise. This keeps
  // the QR (and Share button) present when revisiting a share, not only right
  // after creating one.
  useEffect(() => {
    if (!shareCode) {
      setQrUrl('');
    ...
    let cancelled = false;
    QRCode.toDataURL(`${QR_PREFIX}${shareCode}`, { margin: 1, width: 200 })
      .then((url) => {
  ...
  async function share(access: Access) {
    ...
    try {
      ...
      setCode(c);
      setPanel('closed'); // QR is (re)generated by the effect keyed on shareCode
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleShareQr() {
    if (!qrUrl || !shareCode) return;
    await shareImage(qrUrl, {
      fileName: 'sticker-collector-album-share.png',
      title: `${APP_NAME} shared album`,
      text: `Sync code: ${shareCode} — add it in ${APP_NAME} to join this shared album.`,
    });
  }
      ...
      {mode === 'shared' && isOwner && link && (
        <div className="settings-field" style={{ marginTop: 10 }}>
          ...
          <div className="btn-row" style={{ marginTop: 8 }}>
            ...
            {qrUrl && (
              <button type="button" className="btn full" onClick={() => void handleShareQr()}>
                Share
              </button>
            )}
          </div>
